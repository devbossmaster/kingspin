...
export class AdminOperationsService {
    ...
    return this.pageResult(
      rounds.map((round) => {
        ...
        return {
          ...
          grossPoolAmount: round.totalEntryAmount.toString(),
          platformFeeAmount: round.houseFeeAmount.toString(),
          payoutAmount: round.payoutAmount.toString(),
          netPrizeAmount: round.payoutAmount.toString(),
          platformFeeBps: round.platformFeeBps ?? 0,
  ...
  getSettingsSummary() {
    ...
    return {
      ...
      depositMinimum: String(env.DEPOSIT_MIN_ETB),
      depositMaximum: String(env.DEPOSIT_MAX_ETB),
      withdrawalMinimum: String(env.WITHDRAWAL_MIN_ETB),
      withdrawalMaximum: String(env.WITHDRAWAL_MAX_ETB),
      transferMinimum: String(env.TRANSFER_MIN_ETB),
      transferMaximum: String(env.TRANSFER_MAX_ETB),
      platformFeeBps: String(env.PLATFORM_FEE_BPS),
      platformFeePercent: `${env.PLATFORM_FEE_BPS / 100}%`,
      entryCutoffBufferMs: String(env.ROUND_ENTRY_CUTOFF_BUFFER_MS),
